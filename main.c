...
void printHeader() {
    ...
    printf(GREEN "1 - Afiseaza produse disponibile\n" RESET);
    printf(BLUE "2 - Rezerva produs\n" RESET);
    printf(RED "3 - Anuleaza rezervare\n" RESET);
    printf(YELLOW "4 - Cauta produs\n" RESET);
    printf(MAGENTA "5 - Afiseaza rezervari\n" RESET);
    printf(GREEN "6 - Adauga produs nou\n" RESET);
    printf(RED "0 - Iesire\n" RESET);
    printf(CYAN "=====================================\n" RESET);
}
...
void addProduct() {
    ...
    if (product_count >= MAX_PRODUSE) {
        printf(RED "Nu mai pot fi adaugate produse!\n" RESET);
        return;
    }
    ...
    while (scanf("%f", &p.price) != 1 || p.price <= 0) {
        printf(RED "Pret invalid! Introdu un numar pozitiv: " RESET);
        while (getchar() != '\n');
    }

    printf("Stoc initial: ");
    while (scanf("%d", &p.stock) != 1 || p.stock < 0) {
        printf(RED "Stoc invalid! Introdu un numar pozitiv: " RESET);
        while (getchar() != '\n');
    }

    do {
        printf("Data expirarii (ZZ/LL/AAAA): ");
        scanf("%10s", p.expiry_date);
        if (!validateDate(p.expiry_date)) {
            printf(RED "Format invalid! Foloseste ZZ/LL/AAAA\n" RESET);
        }
...
void reserveProduct() {
    ...
    for (int i = 0; i < product_count; i++) {
        if (products[i].id == product_id) {
            found = 1;
            printf("Produs selectat: " GREEN "%s" RESET " (Stoc: %d)\n",
            ...
            do {
                ...
                if (r.quantity <= 0) {
                    ...
                    printf(RED "Stoc insuficient! Disponibil: %d\n" RESET, products[i].stock);
                }
            ...
            strcpy(r.reservation_date, "01/01/2023");
